...
Before creating a calendar the distribution of the data must be inspected.
...
observed to be ripe, or the 'first ripe fruit' as in the Woodland Trust's Nature's
Calendar project.

There is a further complication in that the ripe fruits of some tree species
persist through winter, such as Holly, and so observations which are classed as
...
boxplot <- ggplot2::ggplot(fruiting_r_trimmed, ggplot2::aes(x = yday, 
...
fruiting_r_cleaned <- fruiting_r_trimmed |>
  dplyr::mutate(
    status = dplyr::case_when(
      ...
      # species == "Populus nigra subsp. betulifolia"
...
calendar <- ggplot2::ggplot(fruiting_r_cleaned, 
              ...
              ggplot2::theme(axis.text.x = ggplot2::element_text(angle = 45, vjust = 1, hjust = -0.15),
                             ...
                             # plot.background = ggplot2::element_rect(fill = '#FFFFFA'),
...
```{r save_ridgeline, message=FALSE, warning=FALSE, include=FALSE, echo=FALSE}
